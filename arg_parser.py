...
def get_args():
    ...
    parser.add_argument("--use_memory_embedding", type=str2bool, default=False,
    ...
    parser.add_argument("--time_encoding", type=str2bool, default=True,
                        help="whether to use time encoding")
    parser.add_argument("--distance_encoding", type=str2bool, default=False,
                        help="")
    parser.add_argument("--de_shortest_path", type=str2bool, default=True,
                        help="for distance_encoding only, whether to use shortest_path")
    parser.add_argument("--de_random_walk", type=str2bool, default=True,
                        help="for distance encoding only, whether to use random_walk")
    parser.add_argument("--edge_attribute", type=str2bool, default=False,
                        help="whether to use edge attribute when generating node embedding")
    parser.add_argument("--use_msg", type=str2bool, default=False,
    ...
    parser.add_argument("--mse_loss_coefficient", type=int, default=2,
                        help="loss weight for the mse loss used in time prediction")


    parser.add_argument("--dyrep_loss", type=str2bool, default=False,
                        help="whether to adopt dyrep-style loss function")
    # parser.add_argument("--other_loss", default="time_prediction", choices=["link_prediction", "time_prediction"],
    #                     help="the adopted loss function (if --dyrep_loss is False),"
    #                          "choices=[\"link_prediction\", \"time_prediction\"].")
    parser.add_argument("--use_dyrepabstractsum", type=str2bool, default=True,
    ...
    parser.add_argument("--basis", default="powerlaw", choices=["exp", "sigmoid", "cos", "relu", "powerlaw", "mixed"],
                        help="the type of basis function adopted in time predictor")

    parser.add_argument("--use_neighbor_finder", type=str2bool, default=True,
                        help="whether to use neighbor_finder (if set to True) or neighbor_loader (if set to False)")

    parser.add_argument("--use_neg", type=str2bool, default=False,
                        help="whether to insert negative samples into batches")
    parser.add_argument("--sample_neg_dst_strictly", type=str2bool, default=True,
                        help="whether to sample negative dst nodes strictly (avoiding coincident positive samples)")
    parser.add_argument("--can_see_neighbors_in_same_batch", type=str2bool, default=True,
                        help="whether the model can see a node's temporal neighbors that is in the same batch with it")
    parser.add_argument("--only_predict_last_events_time", type=str2bool, default=True,
                        help="whether only to predict the last time of the same events in a batch")

    parser.add_argument("--use_memory", type=str2bool, default=True,
    ...
    parser.add_argument("--cpu_only", type=str2bool, default=True,
